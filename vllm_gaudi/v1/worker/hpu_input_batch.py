...
class InputBatch:
            ...
            if sampling_params.allowed_token_ids:
                ...
                if self.allowed_token_ids_mask_cpu_tensor is None:
                    ...
                    self.allowed_token_ids_mask = torch.zeros(self.max_num_reqs,
                                                              ...
                                                              dtype=torch.bool,
                                                              device=self.device)
                    self.allowed_token_ids_mask_cpu_tensor = torch.zeros(self.max_num_reqs,
                                                                         self.vocab_size,
                                                                         dtype=torch.bool,
                                                                         device="cpu")
    ...
    def _make_sampling_metadata(self) -> SamplingMetadata:
        ...
        if not self.all_greedy:
            temperature = async_h2d_copy(self.temperature_cpu_tensor, self.temperature)[:num_reqs]
        else:
            temperature = None
        ...
        if not self.no_allowed_token_ids:
            ...
            async_h2d_copy(self.allowed_token_ids_mask_cpu_tensor, self.allowed_token_ids_mask)
        ...
        return SamplingMetadata(
            ...
            frequency_penalties=None if self.no_penalties else self.frequency_penalties[:num_reqs],
            presence_penalties=None if self.no_penalties else self.presence_penalties[:num_reqs],
            repetition_penalties=None if self.no_penalties else self.repetition_penalties[:num_reqs],
