...
case "$ROLE" in
  ...
  logits)
    ...
    mkdir -p "$DIAG_DIR" && chmod 700 "$DIAG_DIR"
    (umask 077; : > "$DIAG_DIR/ENABLED")
    "$PY" "$REPO/tools/diag_window_driver.py" logits --prompt "${LOGITS_PROMPT:-prose}" \
      --base "http://127.0.0.1:$PORT/v1" --out "$RUNDIR/diag_logits_${STAMP}.json"
    sleep 3
